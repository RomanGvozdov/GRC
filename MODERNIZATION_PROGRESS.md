...
- Тести: **89 pytest** (SQLite), усі зелені. Фронтенд збирається без помилок.
- Alembic head = **`0008`** (без міграції — усе обчислюване/дані/String-enum).
- **Інкременти 1–4 (RMF-конвеєр) + AI-сценарії 1–4 — зроблено. Модернізація за ТЗ
  завершена.** EN-UI пропущено (рішення користувача). Лишилось суто опційне:
  OSCAL-імпорт офіційного 800-53B (чекає файл), PDF-експорт артефактів.
