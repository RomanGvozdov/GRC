...
- Тести: **102 pytest** (SQLite), усі зелені. Фронтенд збирається без помилок.
- Alembic head = **`0008`** (без міграції — усе обчислюване/дані/String-enum).
- **Інкременти 1–4 + AI 1–4 + генератор профілів (ODP) + власні заходи +
  автоматизована звітність — зроблено.** EN-UI пропущено. Лишилось опційне:
  OSCAL-імпорт 800-53B, база знань CIS (чекає файл), RMF-кокпіт, RMF-сповіщення.
- Каталог НД ТЗІ — версія **`2024.r5.1`** (+ODP-параметри). На проді ODP додаються або
  через refresh каталогу, або через **бекфіл** `seed_catalog_parameters` (ідемпотентний,
  не чіпає профілі) — тож параметри з'являться навіть якщо профілі вже побудовані.
...
| (цей) | **Інтеграція з SIEM Wazuh.** `services/wazuh.py` (менеджер :55000 JWT — статус/агенти; indexer :9200 — агрегації подій `wazuh-alerts-*`: всього/за рівнями/топ-правила/топ-агенти/динаміка). API `app/api/integrations.py`: `GET /integrations/wazuh/status` (адмін), `POST /integrations/wazuh/analyze` (controls:read; `save_evidence` → controls:write) — аналіз за період (1–90 діб) + опц. висновок локального LLM (`AISuggestion` kind=`siem`) + опц. збереження як **автоматизований доказ AU-6** (source=`wazuh`, строк дії 30 діб → живить ConMon). Конфіг `WAZUH_*` (у т.ч. `WAZUH_CA_BUNDLE` для самопідписаних TLS). Фронт: картка «SIEM Wazuh» на «Оцінювання та ConMon». `Evidence.requirement` relationship. Тести `test_wazuh.py` (5, клієнт мокається). DEPLOYMENT.md §6. |
| (цей) | **Автоматизована RMF-звітність.** `GET /reports/profile/{id}/soa?fmt=pdf|xlsx` — Декларація застосовності (SoA) за профілем: контролі (застосовно/виключено), походження, статус впровадження (з SSP), ODP-значення (з позначкою «потребує визначення»), обґрунтування tailoring. `GET /reports/system/{id}/readiness` — картка готовності ІКС до авторизації (профіль→SSP→оцінювання→POA&M→ConMon + вердикт authorized/conditional/not_ready). Шаблони `soa_profile.html`, `readiness.html` (WeasyPrint). Фронт: кнопки «SoA PDF/XLSX» на профілі, «Готовність» на ІКС. Тести `test_reports_rmf.py`. |
| (цей) | **Генератор профілів (ODP) + власні заходи захисту.** Витяг ODP-параметрів із базових профілів у каталог (`build_catalogs.parameters_by_code`): 288 параметрів на 105 контролях — **org-defined** (організація заповнює, без default) і **прописані значення** (`default_value`, мінімум за НД ТЗІ). Сід створює `ControlParameter` + ідемпотентний бекфіл `seed_catalog_parameters` (прод-safe). Генератор профілю **передзаповнює прописані значення** в `ProfileParameterValue`; `_resolve` віддає `org_defined`/`needs_input`. Заповнення org-defined — `PUT /profiles/{id}/parameters/{pid}` (без обґрунтування). **Власний захід** — `POST /profiles/{id}/custom-control` (код/назва/текст/ODP в оформленні базового профілю → каталог «nd-tzi-custom» + у профіль origin=added). Фронт: редагування ODP у вкладці «Резолвлене» + форма «Власний захід». Тести `test_odp_generator.py`, `test_custom_control.py`. |
| (цей) | **AI-сценарій 4: агентний RMF-bootstrap §9.** `POST /api/ai/agent/bootstrap-ics/{id}`: детермінований конвеєр за один тригер — профіль (із категоризації/baseline, перевикор. наявний) → SSP → AI-наративи перших N контролів → POA&M з прогалин → AI-резюме. Усе **чернетки**, нічого не затверджується; кроки + провенанс (`AISuggestion` kind=`agent`, `log_action`). Фронт: «✨ AI RMF» на сторінці систем (модалка з результатом і наступними кроками). Тести `test_ai_agent.py`. КСЗІ: лише локальний LLM. |
| (цей) | **AI-сценарій 3: зіставлення + рекомендація контролів §9.** `POST /api/ai/map-control` (вимога → відповідники в іншому каталозі: RAG + точний збіг коду + пояснення LLM) та `POST /api/ai/recommend-controls` (ризик → доречні контролі каталогу за описом загрози/вразливості). Кандидати детерміновані, пояснення від LLM; провенанс `AISuggestion` (kind=`map`/`recommend`); дорадчо (зв'язування вручну). Фронт: «↔ AI» на сторінці «Каталоги» (з вибором цільового каталогу) і «✨ AI: рекомендувати контролі» на сторінці ризику. Тести `test_ai_map_recommend.py`. |
