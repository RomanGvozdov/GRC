...
- Тести: **94 pytest** (SQLite), усі зелені. Фронтенд збирається без помилок.
- Alembic head = **`0008`** (без міграції — усе обчислюване/дані/String-enum).
- **Інкременти 1–4 + AI-сценарії 1–4 + генератор профілів (ODP) + власні заходи —
  зроблено.** EN-UI пропущено. Лишилось опційне: OSCAL-імпорт 800-53B, PDF-експорт,
  база знань CIS (чекає файл), RMF-кокпіт, автоматизована звітність (SoA).
